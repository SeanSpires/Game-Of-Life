...
namespace GameOfLife
{
    public class World
    {
        ...
        public void CreateGrid(int gridHeight, int gridWidth, Cell[,] cells)
        {
            Grid = new Grid(gridHeight, gridWidth) {Cells = cells};
        }

        private int CountNumberOfLiveNeighbours(IEnumerable<Cell> neighbouringCells)
        {
            var numberOfLiveNeighbours =
                neighbouringCells.Count(neighbourCell => neighbourCell.CellState == State.Live);
        ...
        private State FindNextStateOfCell(State cellState, int numberOfLiveNeighbours)
        {
            ...
            {
                if (numberOfLiveNeighbours < 2) return State.Dead;

                if (numberOfLiveNeighbours > 3) return State.Dead;
            }
            else
            {
                if (numberOfLiveNeighbours == 3) return State.Live;
            }
